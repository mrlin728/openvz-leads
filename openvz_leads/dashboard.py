...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── Forms ── */
  /* ── Target: the natural-language entry point ── */
  .target-input {
  ...
  .target-examples { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
  .target-example {
    background: rgba(255,255,255,0.03); border: 1px solid var(--border);
    border-radius: 999px; padding: 5px 12px; font-size: 12px;
    color: var(--text-2); cursor: pointer; font-family: inherit;
  }
  .target-example:hover { color: var(--text); border-color: var(--border-strong); }
  .draft-grid {
    display: grid; grid-template-columns: max-content 1fr;
    gap: 10px 18px; margin: 4px 0 16px; font-size: 13px; align-items: baseline;
  }
  .draft-grid dt { color: var(--text-3); }
  .draft-grid dd { color: var(--text); }
  .chip {
    display: inline-block; background: var(--accent-soft);
    border: 1px solid rgba(62,207,142,0.25); color: var(--accent);
    border-radius: 6px; padding: 2px 9px; font-size: 12px; margin: 0 5px 5px 0;
  }
  .chip.plain {
    background: rgba(255,255,255,0.04); border-color: var(--border-strong);
    color: var(--text-2);
  }
  .assumptions {
    border-left: 2px solid var(--amber); padding: 4px 0 4px 14px;
    margin: 18px 0; font-size: 13px; color: var(--text-2); line-height: 1.65;
  }
  .assumptions .head { color: var(--amber); font-weight: 600; }
  .assumptions li { list-style: none; margin-top: 6px; }
  .assumptions li::before { content: "· "; color: var(--text-3); }

  /* ── Funnel ── */
  .funnel-row {
    display: grid; grid-template-columns: 96px 46px 1fr;
    align-items: center; gap: 14px; padding: 9px 0;
    border-bottom: 1px solid var(--border);
  }
  .funnel-row:last-child { border-bottom: none; }
  .funnel-row .name { font-size: 13px; font-weight: 600; }
  .funnel-row .n {
    font-size: 14px; font-variant-numeric: tabular-nums; text-align: right;
  }
  .funnel-row .n.zero { color: var(--text-3); }
  .funnel-track { display: flex; align-items: center; gap: 10px; }
  .funnel-bar { height: 7px; border-radius: 4px; background: var(--accent-deep); min-width: 2px; }
  .funnel-row.terminal .funnel-bar { background: #3a4557; }
  .funnel-row.won .funnel-bar { background: var(--accent); }
  .funnel-row.lost .funnel-bar { background: var(--red); }
  .funnel-desc { font-size: 12px; color: var(--text-3); }
  /* Under ~700px the three columns leave the description a sliver, and it
     wraps to two words a line. Stack instead. */
  @media (max-width: 700px) {
    .funnel-row { grid-template-columns: 1fr auto; gap: 4px 12px; }
    .funnel-row .n { grid-column: 2; grid-row: 1; }
    .funnel-track { grid-column: 1 / -1; flex-direction: column; align-items: stretch; gap: 5px; }
    .funnel-bar { width: 100% !important; max-width: 100%; }
  }
...
<div id="target" class="section">
  <div class="section-head">
    <h2 data-i18n="target.title">Who are you looking for?</h2>
    <p data-i18n="target.sub">Say it the way you would say it to a colleague. It becomes the ICP the Scout searches on and the Profiler scores against — after you have seen what it filled in that you never said.</p>
  </div>

  <div class="card">
    <textarea class="target-input" id="target-input" rows="3"
      data-i18n-placeholder="target.placeholder"
      placeholder="Find dental clinics in California with outdated websites and 5-50 employees"></textarea>
    <div class="target-examples" id="target-examples"></div>
    <div class="btn-group">
      <button class="btn btn-primary" id="target-parse" onclick="parseTarget()" data-i18n="target.parse">Read this</button>
      <span id="target-status" style="align-self:center;font-size:12px;color:var(--text-3)"></span>
    </div>
  </div>
  ...
  <div class="card">
    <h2 data-i18n="target.currentTitle">Currently configured</h2>
    <p class="lede" data-i18n="target.currentLede">What the agent searches for right now. Saving above replaces this one block in openvz-leads.yaml — the rest of the file, comments included, is left exactly as it was.</p>
    <div id="target-current"></div>
  </div>

  <div class="card">
    <h2 data-i18n="target.funnelTitle">Where everyone is</h2>
    <p class="lede" data-i18n="target.funnelLede">Meeting, won and lost are yours to set — nothing in an inbox proves a deal closed. Each move is recorded and, if a CRM is configured, pushed to it.</p>
    <div id="target-funnel"></div>
  </div>
</div>
...
const I18N = {
  zh: {
    ...
    'btn.refresh': '刷新',
    'setup.engineHead': '当前引擎',
    'setup.engineBrain': '大脑',
    'setup.engineCrawl': '抓取',
    'setup.engineCrm': 'CRM',
    'setup.check.target.label': '已设定目标客户',
    'setup.check.target.help': '在「定位」页用一句话说清你要找谁，或者跑：openvz-leads target "美国牙科诊所"',
    'crm.none': '未开启',
    'crm.webhook': 'Webhook',
    'crm.file': '写入文件',
    'nav.target': '定位',
    ...
    'target.placeholder': '帮我找美国牙科诊所',
    'target.parse': '读一下这句',
    'target.reading': '正在读…',
    'target.failed': '没能读懂这句话',
    'target.draftTitle': '它是这样理解的',
    'target.industries': '行业',
    'target.size': '规模',
    'target.geography': '地区',
    'target.titles': '职位',
    'target.keywords': '还要满足',
    'target.exclusions': '直接排除',
    'target.assumed': '以下是它替你填的 —— 这些你没说过：',
    'target.confidence': '置信度：{level}',
    'target.viaModel': '由模型解析',
    'target.viaHeuristic': '无模型，按规则解析（会粗糙很多）',
    'target.save': '存为我的 ICP',
    'target.again': '重新读一次',
    'target.saved': '已写入 openvz-leads.yaml',
    'target.currentTitle': '当前生效的定位',
    'target.currentLede': '智能体现在就是按这个在找。上面保存时只替换 openvz-leads.yaml 里的这一段，文件其余部分连注释都原样保留。',
    'target.noConfig': '还没有配置文件。先跑 openvz-leads setup 或 openvz-leads train <你的网址>。',
    'target.funnelTitle': '每个人现在在哪一步',
    'target.funnelLede': '会面、赢单、丢单由你来定 —— 收件箱里没有任何东西能证明一单成了。每一次移动都会记录，配了 CRM 就同步过去。',
    'target.crmLine': 'CRM 同步：{state}',
    'target.unsynced': '{n} 条还没同步',
    'confidence.low': '低',
    'confidence.medium': '中',
    'confidence.high': '高',
    'stage.new': '已找到',
    ...
    'stage.lost': '丢单',
    'stage.opted_out': '已退订',
    'stageDesc.new': '找到了，还没分析也还没写信。',
    'stageDesc.queued': '开发信已起草 —— 等审核，或等发送。',
    'stageDesc.contacted': '确实发出去了。',
    'stageDesc.replied': '对方回了。',
    'stageDesc.meeting': '会面已经约上。',
    'stageDesc.won': '成了。',
    'stageDesc.lost': '没成。',
    'stageDesc.opted_out': '对方要求别再联系。终态。',
  ...
  en: {
    ...
    'btn.refresh': 'Refresh',
    'setup.engineHead': 'The engine',
    'setup.engineBrain': 'Brain',
    'setup.engineCrawl': 'Reading pages',
    'setup.engineCrm': 'CRM',
    'crm.none': 'off',
    'crm.webhook': 'webhook',
    'crm.file': 'file',
    ...
    'target.placeholder': 'Find dental clinics in California with outdated websites and 5-50 employees',
    'target.parse': 'Read this',
    'target.reading': 'Reading…',
    'target.failed': 'Could not read that',
    'target.draftTitle': 'What it understood',
    'target.industries': 'Industries',
    'target.size': 'Size',
    'target.geography': 'Geography',
    'target.titles': 'Titles',
    'target.keywords': 'Must also match',
    'target.exclusions': 'Rules out',
    'target.assumed': 'Filled in for you — you did not say these:',
    'target.confidence': 'Confidence: {level}',
    'target.viaModel': 'parsed by the model',
    'target.viaHeuristic': 'no model available — parsed by rules, which is much rougher',
    'target.save': 'Save as my ICP',
    'target.again': 'Read it again',
    'target.saved': 'Written to openvz-leads.yaml',
    'target.currentTitle': 'Currently configured',
    'target.currentLede': 'What the agent searches for right now. Saving above replaces this one block in openvz-leads.yaml — the rest of the file, comments included, is left exactly as it was.',
    'target.noConfig': 'No configuration yet. Run openvz-leads setup, or openvz-leads train <your-site>.',
    'target.funnelTitle': 'Where everyone is',
    'target.funnelLede': 'Meeting, won and lost are yours to set — nothing in an inbox proves a deal closed. Each move is recorded, and pushed to a CRM if one is configured.',
    'target.crmLine': 'CRM sync: {state}',
    'target.unsynced': '{n} not yet synced',
    'confidence.low': 'low',
    'confidence.medium': 'medium',
    'confidence.high': 'high',
    'stage.new': 'Found',
    'stage.queued': 'Drafted',
    'stage.contacted': 'Contacted',
    'stage.replied': 'Replied',
    'stage.meeting': 'Meeting',
    'stage.won': 'Won',
    'stage.lost': 'Lost',
    'stage.opted_out': 'Opted out',
    'stageDesc.new': 'Found. Not yet analysed or written to.',
    'stageDesc.queued': 'Outreach drafted — waiting on review, or on a send.',
    'stageDesc.contacted': 'Something actually went out.',
    'stageDesc.replied': 'They answered.',
    'stageDesc.meeting': 'A meeting is booked.',
    'stageDesc.won': 'Closed. They bought.',
    'stageDesc.lost': 'Closed. They did not.',
    'stageDesc.opted_out': 'Asked not to be contacted. Final.',
...
// ── Target: a sentence in, an ICP out ──
//
// Two steps on purpose. Parsing shows what was inferred; only the second
// button writes anything. The assumptions list is the whole reason this is
// trustworthy, and it is worth nothing if the config has already changed by
// the time you read it.

let TARGET_DRAFT = null;

const TARGET_EXAMPLES = {
  en: [
    'Find dental clinics in California with outdated websites and 5-50 employees',
    'B2B SaaS companies in the UK that recently raised a Series A',
    'Manufacturing firms in Germany, 50-500 staff, no English website',
  ],
  zh: [
    '帮我找美国牙科诊所',
    '英国刚拿到 A 轮的 B2B SaaS 公司',
    '德国的制造企业，50-500 人，官网没有英文版',
  ],
};

function loadTarget() {
  renderTargetExamples();
  loadCurrentIcp();
  loadFunnel();
}

function renderTargetExamples() {
  const box = document.getElementById('target-examples');
  if (!box) return;
  const list = TARGET_EXAMPLES[LANG] || TARGET_EXAMPLES.en;
  box.innerHTML = list.map(text =>
    '<button class="target-example" data-text="' + escHtml(text) + '">' +
    escHtml(text) + '</button>'
  ).join('');
  // Listener rather than an inline onclick: the example text is user-facing
  // copy with apostrophes in it, and escaping it into an attribute handler
  // is one quoting mistake away from a broken button.
  box.querySelectorAll('.target-example').forEach(el => {
    el.onclick = () => {
...
async function parseTarget() {
  ...
  try {
    ...
    if (!res.ok) { showToast(data.error || t('target.failed'), 'error'); return; }
    TARGET_DRAFT = data.draft;
    renderDraft(data);
  } catch (e) {
    showToast(t('target.failed'), 'error');
  } finally {
...
function chips(values, plain) {
  if (!values || !values.length) return '<span style="color:var(--text-3)">—</span>';
  return values.map(v =>
    '<span class="chip' + (plain ? ' plain' : '') + '">' + escHtml(v) + '</span>'
  ).join('');
}
...
function renderDraft(data) {
  ...
  let html = '<div class="card"><h2>' + escHtml(t('target.draftTitle')) + '</h2>';
  if (draft.summary) {
    html += '<p class="lede">' + escHtml(draft.summary) + '</p>';
  }

  html += '<dl class="draft-grid">' +
    row(t('target.industries'), chips(draft.industries)) +
    row(t('target.size'), draft.company_size
        ? '<span class="chip plain">' + escHtml(draft.company_size) + '</span>'
        : '<span style="color:var(--text-3)">—</span>') +
    row(t('target.geography'), chips(draft.geography, true)) +
    row(t('target.titles'), chips(draft.titles, true)) +
    ((draft.keywords && draft.keywords.length)
      ? row(t('target.keywords'), chips(draft.keywords)) : '') +
    ((draft.exclusions && draft.exclusions.length)
      ? row(t('target.exclusions'), chips(draft.exclusions, true)) : '') +
    '</dl>';

  if (draft.assumptions && draft.assumptions.length) {
    html += '<div class="assumptions"><div class="head">' +
      escHtml(t('target.assumed')) + '</div><ul>' +
      draft.assumptions.map(a => '<li>' + escHtml(a) + '</li>').join('') +
      '</ul></div>';
  }

  html += '<div style="font-size:12px;color:var(--text-3)">' +
    escHtml(tf('target.confidence', {level: t('confidence.' + (draft.confidence || 'low'))})) +
    ' · ' + escHtml(draft.via === 'model' ? t('target.viaModel') : t('target.viaHeuristic')) +
    '</div>';

  if (data.note) {
    html += '<div class="assumptions" style="border-left-color:var(--red)">' +
      escHtml(data.note) + '</div>';
  }

  html += '<div class="btn-group">' +
    '<button class="btn btn-primary" onclick="applyTarget()">' +
      escHtml(t('target.save')) + '</button>' +
    '<button class="btn btn-secondary" onclick="parseTarget()">' +
      escHtml(t('target.again')) + '</button>' +
    '</div></div>';
...
async function applyTarget() {
  ...
  try {
    const res = await fetch('/api/icp/apply', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({draft: TARGET_DRAFT}),
    });
    const data = await res.json();
    if (!res.ok) { showToast(data.error || t('target.failed'), 'error'); return; }
    showToast(t('target.saved'), 'success');
    document.getElementById('target-result').innerHTML = '';
    TARGET_DRAFT = null;
    loadCurrentIcp();
  } catch (e) {
...
async function loadCurrentIcp() {
  const box = document.getElementById('target-current');
  if (!box) return;
  try {
    const data = await (await fetch('/api/icp')).json();
    if (!data.configured) {
      box.innerHTML = '<div class="empty">' + escHtml(t('target.noConfig')) + '</div>';
      return;
    }
    const icp = data.icp || {};
    let html = '';
    if (data.request) {
      html += '<p class="lede" style="margin-top:0">“' + escHtml(data.request) + '”</p>';
    }
    html += '<dl class="draft-grid" style="margin-bottom:0">' +
      row(t('target.industries'), chips(icp.industries)) +
      row(t('target.size'), icp.company_size
          ? '<span class="chip plain">' + escHtml(icp.company_size) + '</span>'
          : '<span style="color:var(--text-3)">—</span>') +
      row(t('target.geography'), chips(icp.geography, true)) +
      row(t('target.titles'), chips(icp.titles, true)) +
      ((icp.keywords && icp.keywords.length)
        ? row(t('target.keywords'), chips(icp.keywords)) : '') +
      ((icp.exclusions && icp.exclusions.length)
        ? row(t('target.exclusions'), chips(icp.exclusions, true)) : '') +
      '</dl>';
    box.innerHTML = html;
  } catch (e) {
    box.innerHTML = '<div class="empty">' + escHtml(t('target.noConfig')) + '</div>';
...
async function loadFunnel() {
  const box = document.getElementById('target-funnel');
  if (!box) return;
  try {
    const data = await (await fetch('/api/pipeline')).json();
    const stages = data.stages || [];
    const peak = Math.max(1, ...stages.map(s => s.count || 0));
    let html = stages.map(stage => {
      const count = stage.count || 0;
      const width = Math.round((count / peak) * 100);
      const classes = ['funnel-row'];
      if (stage.terminal) classes.push('terminal');
      if (stage.id === 'won') classes.push('won');
      if (stage.id === 'lost') classes.push('lost');
      return '<div class="' + classes.join(' ') + '">' +
        '<div class="name">' + escHtml(t('stage.' + stage.id)) + '</div>' +
        '<div class="n' + (count ? '' : ' zero') + '">' + count + '</div>' +
        '<div class="funnel-track">' +
          (count ? '<div class="funnel-bar" style="width:' + width + '%"></div>' : '') +
          '<div class="funnel-desc">' + escHtml(t('stageDesc.' + stage.id)) + '</div>' +
        '</div></div>';
    }).join('');
    const crm = data.crm || {provider: 'none', target: ''};
    let state = t('crm.' + crm.provider);
    if (crm.target) state += ' — ' + crm.target;
    html += '<div style="margin-top:16px;font-size:12px;color:var(--text-3)">' +
      escHtml(tf('target.crmLine', {state: state}));
    // Only when a sync target exists: with it off, "0 synced" is the design,
    // not a backlog.
    if (crm.provider !== 'none' && data.unsynced) {
      html += ' · ' + escHtml(tf('target.unsynced', {n: data.unsynced}));
    }
    html += '</div>';
    box.innerHTML = html;
  } catch (e) {
...
async function loadSetupStatus() {
  ...
  if (engine.brain) {
    // Three lines, not a tick each: these are settings rather than steps —
    // every one has a working default, and what matters is knowing which
    // one is in force before wondering why a run behaved as it did.
    html += '<div class="subhead">' + escHtml(t('setup.engineHead')) + '</div>';
    html += engineRow(t('setup.engineBrain'), engine.brain,
...
function engineRow(label, value, problem) {
  const icon = problem
    ? '<span class="check-icon pending">&#9679;</span>'
    : '<span class="check-icon done">&#10003;</span>';
  return '<div class="check-item">' + icon +
    '<div class="check-info">' +
      '<div class="check-label ' + (problem ? '' : 'done') + '">' +
        escHtml(label) + ' — ' + escHtml(value) + '</div>' +
      (problem ? '<div class="check-help">' + escHtml(problem) + '</div>' : '') +
    '</div></div>';
}
