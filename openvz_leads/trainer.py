"""Trainer — give OpenVZ Leads a URL and it learns everything about the product.
...
class Trainer:
    ...
    async def train(
        self,
        url: str,
        output_path: str = "openvz-leads.yaml",
        max_pages: int = 100,
    ):
        """Train OpenVZ Leads on a product website.
        ...
        logger.info(f"Training OpenVZ Leads on {base_url}...")
        print(f"\n{'='*60}")
        print(f"  OpenVZ Leads Training Mode")
        ...
        print(f"  Then run: openvz-leads run\n")
...
Be thorough — you have access to the full website. Extract everything relevant."""
        ...
        result = await self.brain.think_json(prompt, session_id="leads-trainer")
...
"""
    ...
    def _build_config(
        ...
        """Build the complete openvz-leads.yaml config."""
        ...
        # "www.acme.com" should yield hello@acme.com, not hello@www.acme.com
        ...
        return {
            "persona": {
                ...
                "email": f"hello@{email_domain}",
                "linkedin": f"linkedin.com/in/contact-{email_domain.replace('.', '-')}",
                "tone": tone,
            },
            "product": {
                "name": product,
...
async def run_training(url: str, output: str = "openvz-leads.yaml", max_pages: int = 100):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m openvz_leads.trainer <website-url> [max-pages]")
        print("")
        print("Examples:")
        print("  python -m openvz_leads.trainer https://acmecorp.com")
        print("  python -m openvz_leads.trainer https://acmecorp.com 200")
        print("")
        ...
        print("")
        print("Without Cloudflare credentials, OpenVZ Leads will use basic HTTP")
